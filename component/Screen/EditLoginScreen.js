...
const EditLoginScreen = (props) => {
  ...
  useEffect(() => {
    let obj = data.findIndex((x) => {
      return x.image == props.route.params.userDetails.user_image;
    });
    if (obj >= 0) {
      setSelectedProfile(obj);
    }
    setUserName(props.route.params.userDetails.user_name);
    console.log(props.route.params.userDetails, 'ffdf', obj);
  ...
  function goToQuestionScreen() {
    ...
    if (netInfo.isConnected) {
      let obj = {
        user_name: userName,
        uuid: deviceId,
        user_id: props.route.params.userDetails.user_id,
        user_image: selectedProfile >= 0 ? data[selectedProfile].image : null,
      };
      let newObj = props.route.params.userDetails;
      newObj.user_name = userName;
      newObj.user_image = data[selectedProfile].image;
      HomescreenService.addUser(obj)
        .then((response) => {
          ...
          if (data.status) {
            ...
            storeData('User', newObj).then((storRes) => {
        ...
        <KeyboardAvoidingView>
                ...
                return (
                  <Animated.View // Special animatable View
                    style={{
                      ...
                      backgroundColor:
                        index == selectedProfile ? 'green' : 'red',
          ...
          <TouchableOpacity
            ...
            <View
              ...
              <Text style={styles.levelStyle1}>Update</Text>
            </View>
          </TouchableOpacity>
        </KeyboardAvoidingView>
      </LinearGradient>
